...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMEssage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_RBUTTONDOWN:
	{
		...
		if (xClick == rect.left + 20 || xClick == rect.right - 20 || yClick == rect.bottom - 20 || yClick == rect.top + 20)
			wsprintf(str, L"Frame X: %d Y: %d",xClick, yClick);
		else if (xClick > rect.left + 20 && xClick < rect.right - 20 && yClick<rect.bottom - 20 && yClick>rect.top + 20)
			wsprintf(str, L"IN X: %d Y: %d", xClick, yClick);
		else

			wsprintf(str, L"OUT X: %d Y: %d", xClick, yClick);
